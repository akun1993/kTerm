                                    ...
                                    int cellwidth, bool combining)
{
    ...
    while (utflen > 0) {
        ...
        char *buff[1024];
        memcpy(buff,utfptr,clen);
        buff[clen] = '\0';
        // printf(
        //     "drawInternal:%s combining %s n %d cellwidth %d hadvance %d y %d fontHeight %d "
        //     "fm.height %d\n ",buff, combining ? "true" : "false",
        //     n, cellwidth, rect.width,y,pfont->u.height,rect.height);                    
        if (shadowbold)
            draw_layout(ctx,
