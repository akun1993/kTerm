    ...
    unsigned long attr, int lattr, truecolour truecolour)
{
    ...
    if (truecolour.bg.enabled){
        ...
        printf("bg color %d is 0x%02x%02x%02x \n",nbg,truecolour.bg.r,truecolour.bg.g,truecolour.bg.b);  
    }
    else{
        draw_set_colour(inst, nbg, attr & ATTR_DIM);
        printf("bg color %d is 0x%02x%02x%02x \n",nbg, inst->cols[nbg].red,inst->cols[nbg].green,inst->cols[nbg].red );  
    }
    ...
    if (truecolour.fg.enabled){
        ...
        printf("fg color %d is 0x%02x%02x%02x \n",nfg,truecolour.fg.r,truecolour.fg.g,truecolour.fg.b);  
    }
    else {
        draw_set_colour(inst, nfg, attr & ATTR_DIM);
        printf("fg color %d is 0x%02x%02x%02x \n",nfg,inst->cols[nfg].red,inst->cols[nfg].green,inst->cols[nfg].red ); 
    }
    if (ncombining > 1) {
        assert(len == 1);
    ...
    int utflen;
    char *utfstring = dup_wc_to_mb_c(CS_UTF8, text, len, "", &utflen);
    printf("%s rect(%d %d %dx%d)\n", utfstring,x*inst->font_width+inst->window_border,
                   y*inst->font_height+inst->window_border,
                   rlen*widefactor*inst->font_width, inst->font_height); 
    if (attr & ATTR_UNDER) {
